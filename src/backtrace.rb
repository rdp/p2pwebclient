...
# TODO only do the backtrace if last command was 'raise'
	 ...
	 if args
	   for arg in args
		  ...
		  value =  eval(arg, previous_frame_binding)
                        ...
                        # astoundingly, calls to an aliased require somehow get extra parameters--at least I think so
                        # puts "bad arg", arg # mostly path, options
                end
		collected << [arg, value]
	   end 
	 else
   ...
   Thread.critical = saved_crit
	...
	print "BAD" + e.to_s + e.backtrace[0..1].inspect
   end
  end
  ...
  at_exit do # at_exit seems to be run by the last running Thread. Or an exiting thread? Not sure for always.
    ...
    for loc, params, binding in Thread.current['backtrace'].reverse do
        original_line = get_line( loc[0], loc[1]).strip
