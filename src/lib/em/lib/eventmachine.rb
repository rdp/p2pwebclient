# $Id: eventmachine.rb 697 2008-06-15 22:00:00Z francis $
#
...
module EventMachine
	...
	def EventMachine::run blk=nil, tail=nil, &block
		...
		if reactor_running?
			...
			begin
				...
				print "EM interrupted #{e} #{e.class} #{e.backtrace.join("\n")}"
				print "run ulimit?" if e.class == Errno::EMFILE
				#sleep if Socket.gethostname =~ /roger.*macbook/
			ensure
				print "EM done #{$!}\n"
				release_machine
  ...
  def EventMachine::add_periodic_timer *args, &block
	...
	private_class_method :cancel_timer # only EM methods can call this
...
class Connection
  ...
  #
	...
	#
	def receive_data data
		puts "receive_dataed............>>>#{data.length}"
	end
