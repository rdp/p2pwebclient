...
class Array
  ...
  def interpret_args *settings_array
   ...
   # settings_array will be [], :optional_name, value, :optional_name2, value2
    ...
    unless ending_hash.empty? 
	   raise MalformedArguments.new("unknown vars! #{ending_hash.keys.inspect} not in #{settings_array.inspect} -- did you forget a colon?") if ending_hash.keys[0].class != Symbol
	   raise MalformedArguments.new("unknown vars! #{ending_hash.keys.inspect} not in #{settings_array.inspect}") 
    end
