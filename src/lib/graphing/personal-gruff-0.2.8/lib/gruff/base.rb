...
class Numeric
  # doctest: with_commas adds commas to strings
  # >> 1000.with_commas
  # => '1,000'
  # >> 100.with_commas
  # => '100'
  # >> 1000000.with_commas
  # => '1,000,000'
  # >> 20000000000.with_commas
  # => '20,000,000,000'
  # >> 1000.0.with_commas
  # => '1,000.0'
  # >> 1000.0006.with_commas
  # => '1,000.0006'
  def with_commas
    output = self.to_s
    that_past_decimal = ''
    if output.index('.')
      output, that_past_decimal = output.split('.')
      that_past_decimal = '.' + that_past_decimal
    end
    original_length = output.length
    ((original_length - 1)/3).times { |n|
      
      output.insert(original_length  - ((n + 1)*3), ',')
    }
    output + that_past_decimal
  end
end
module Gruff
...
#      theme_odeo
    ...
    def initialize_ivars
      ...
      @data = Array.new # lodo double check :)
      @has_data = false
      @labels = Hash.new
      @labels_seen = Hash.new
      @sort = true # lodo shouldn't this be false?
      @title = nil
    ...
    def theme_white_green_only
      self.theme = {
        :colors => ['#aedaa9'], # 3077a9 blue, aedaa9 light green
        :marker_color => '#dddddd',
        :font_color => 'black',      
        :background_colors => "white"
        # :background_image => File.expand_path(File.dirname(__FILE__) + "/../assets/backgrounds/43things.png")
    ...
    def data(name, data_points=[], color=nil)
      ...
      # was 0
      @column_count = [@column_count, data_points.length].max
      ...
      data_points.each_with_index do |data_point, index|
        ...
        @has_data = true if @maximum_value > 0 # lodo fails with [0,0] graphs. ugh :)
    ...
    def write(filename="graph.png")
      ...
      begin
        ...
        print "emergency writing it to " + filename + ".gif"
        begin
          @base_image.write(filename + ".gif")
        rescue
          print "emergency write failed"
        else
          print "emergency write succeeded"
        end
      end
...
protected
    ...
    def draw shouldNormalize = true
      make_stacked if @stacked
      setup_drawing shouldNormalize
    ...
    def setup_drawing normalizeYes
      ...
      define_left_side_labels() # MODIFIED
    ...
    def normalizePoint(rawY)
      return ((rawY.to_f - @minimum_value.to_f ) / @spread)
    end
    ...
    def setup_graph_measurements
      ...
      if @hide_line_markers
        ...
        if @has_left_labels
          ...
          longest_left_label_width = calculate_width(@marker_font_size, 
                          label(@largest_marker_line_text || @maximum_value.to_f))
        end
        ...
        if !@labels.nil? and !labels.empty?
            last_label = @labels.keys.sort.last # lodo do we need a .to_i? I wouldn't think so...
      
            # lodo it should check for the last 'few', really, as the last most one might overlap no matter what!
    
            # see if we want to make it 'larger' to fit the last right most label
            # let's see...guarantee it fits for sure
            # we want to calculate the 'central spot' for the last label
            #if the last label is at the end edge
            if (last_label >= rightMostPoint) && @center_labels_over_point
      ...
      end # go ahead and calculate the other metrics, tho
    ...
    def define_left_side_labels
      if @y_axis_increment.nil? # then it hasn't been spread yet :)
        # Try to use a number of horizontal lines that will come out even.
        #
        # TODO Do the same for larger numbers...100, 75, 50, 25 # THIS IS WHERE it tries to pick clean numbers for the
        if @marker_count.nil?
          (3..7).each do |lines|
            if @spread % lines == 0.0 # so this just says...try to find an even multiple to use for the left numbers.
              @marker_count = lines
              break
            end
          end
        ...
        # lodo make this work with mine :)
        @maximum_value = [@maximum_value.ceil, @y_axis_increment].max
        @minimum_value = @minimum_value.floor
        calculate_spread
        # lodo this just 'made sure' it was normalized so that axes fit -- lodo check :)
        normalize(true)
        
        @marker_count = (@spread / @y_axis_increment).to_i
        @increment = @y_axis_increment
      end
      # this is our pre-emptive "figure out longest right here" -- TODO refactor to a single method?
      @largest_marker_line_text = ''
    ...
    def significant(inc)
      if "%.02f" % inc == "0.25" # hack for CDF's to work todo fix
        return 0.25 # it was .25something so why not? lodo take off, investigate why sometimes works, sometimes doesn't...egh
      end
...
private
    ...
    def label(value)      
      if (@spread.to_f % @marker_count.to_f == 0) || !@y_axis_increment.nil?
        return value.gsub(',','_').to_i.with_commas
      end
      
      if @spread > 10.0
        sprintf("%0i", value.gsub(',', '_').to_i).to_i.with_commas
      elsif @spread >= 3.0
        sprintf("%0.2f", value.gsub(',', '_').to_f).to_f.with_commas
      else
        value.to_s
      end
    end 
