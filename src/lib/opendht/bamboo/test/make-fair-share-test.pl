...
my $client_count = 2;
my $seed = 1;
my $end_time = 300*60*1000;
my $client_start_time = 1*60*1000;
my @client_rates = (1000, 100);
my $put_size = 512;
my $put_ttl_sec = 3600;

srand $seed;

open (EXP, ">/tmp/fair-share-test.exp") or die;
    ...
    print CFG<<EOF;
        ...
        <Dht>
            ...
            <initargs>
                ...
                virtual_disks 3
                disk_size_0 1048576 # 1 MB
                token_bucket_size_0 10240 
            </initargs>
        </Dht>
...
for (my $i = 1; $i <= $client_count; ++$i) {
    ...
    my $gateway = $possible_gateways[$which];
    my $ip;
    while (1) {
        my $graph_index = int (rand () * 128) + 1;
        $ip = "10.0.0.$graph_index";
        if (! (defined $client_ipset{$ip})) {
            $client_ipset{$ip} = $ip;
            last;
        }
    }
    my $cfg = "/tmp/client-$i.cfg";
    my $rate = $client_rates[$i%($#client_rates+1)];
    printf EXP "$ip:$port\t%19s %8d %10d\n", $cfg, 
    ...
    <stages>
        <GatewayClient>
            ...
            <initargs>
                debug_level             0
EOF
    print CFG "            gateway $gateway:3662\n";
    print CFG<<EOF;
            </initargs>
        </GatewayClient>
...
EOF
    print CFG "            seed $i\n";
    print CFG "                mean_wait_ms            $rate\n";
    print CFG "                put_size                $put_size\n";
    print CFG "                put_ttl_sec             $put_ttl_sec\n";
    print CFG<<EOF;
