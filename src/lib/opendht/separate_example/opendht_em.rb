...
class OpenDHTEM
  ...
  if use_opendht
    ...
    StartingGateways = [['130.92.70.251', 3633]]
    Opendht_servers_file ="servers.txt"
  elsif Socket.gethostname =~ /roger.*macbook/
    StartingGateways = [['localhost', 3633]]
    Opendht_servers_file ="../cached_all_gateways_file_name_roger_macbook"
  else # the default
    StartingGateways = [['planetlab3.flux.utah.edu', 3633]]# central non redundant points of failure are bad TODO
    Opendht_servers_file ="lib/opendht/cached_all_gateways_file_name"
  end

  def initialize logger, key_multiply_redundancy = 1, max_num_simultaneous_gateway_requests_per_query = 1, gateway_pool_size = 1, gateway_pool_creation_race_size = 0 # these typically come back very quicky, but still...we'll keep pool size and pool creation race size close in number to free up some descriptors for ourselves :)
    raise "max_num_simultaneous_gateway_requests_per_query must be a multiple of key_multiply_redundancy" if max_num_simultaneous_gateway_requests_per_query % key_multiply_redundancy != 0
    @logger = logger
    @log_prefix = "odhtem"
    ...
    @secret = 'secret' # lame :)
    @ttl = 5*60
    @application ='hm' # stands for happy meal.
  ...
  # => 5
  # doctest: you can tell it to stop
  ...
  def calculate_new_live_gateways total_fast_to_try
    ...
    unless defined?(@allServerArrays)
      ...
      @allServerArrays = allServerArrays
    end
        ...
        EM::connect(server, port, SingleConnectionCompleted) { |conn|
          ...
          conn.connection_completed_block = proc { 
            if total_got_so_far < total_fast_to_try
              total_got_so_far += 1
              print "adding fast gw #{incoming.inspect} -- size now #{@gateways.length + 1}"
              if @gateways == @original_gateways
                @gateways = [incoming]
              else
                @gateways << incoming
              end
  ...
  def do_single_key_single_action key_new, round_id, description, repeat_extra_times_past_this, on_done_proc, &block
   dbg unless block
    ...
    ODHTTransport.setup(self, description, round_id, key_new, on_done_proc) { |conn, host, port| # ltodo rename on_done_proc same for all func's that have one
      ...
      uid = "#{description.gsub(':', '_')} ##{@@uid_per_transaction += 1}"
      ...
      conn.set_finalize_function proc { |return_status, values, key_used| 
        ...
        if return_status == :success or repeat_extra_times_past_this == 0 
          ...
          error "repeating for erring! #{description} #{repeat_extra_times_past_this}"
  ...
  def add(key, value, round_id = key + value.to_s + rand(1000000).to_s, description = '', block_proc = nil, retry_times = @retry_times_if_non_get, &block)
...
class ODHTTransport < EventMachine::Connection
  @@timeout_considered_error_in_seconds = 2*60   # these do timeout, too
  ...
  def ODHTTransport.setup(parent, description, round_id, key_new, all_the_way_out_proc)
    ...
    begin
      ...
      parent.logger.error "SEEDO BAD! opendht erred in an odd way #{round_id} #{e.to_s + e.class.to_s} #{gw.inspect} #{e.backtrace.inspect}" rescue print "yipes #{e} "
  ...
  def set_finalize_function proc_to_call_on_return # used
    dbg unless proc_to_call_on_return.class == Proc
